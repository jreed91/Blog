...
# Configuration
TUMBLR_DOMAIN =   "http://jreed91.tumblr.com"
WRITE_DIRECTORY = "source/_posts"
  ...
  open(path) do |xml|
    ...
    posts.each do |post_tag|
      ...
      if type == "regular"    
        ...
        title = post_tag.css("quote-source").first.content
        ref_url = post_tag.css("quote-source_url").first # TO FIX: figure out correct syntax to grab source_url
        body = "<blockquote>#{text}</blockquote>"
        tags << "quote"
      elsif type == "link"
        text_tag = post_tag.css("link-text").first
        text = text_tag.nil? ? nil : text_tag.content
        ref_url = post_tag.css("link-url").first.content
